...
struct CheckIfXCanGoRight
{
    ...
    typedef typename ConditionalAndValue<
        ...
        ConditionalAndValue<!current_place_ok, ValueWrapperBool<false>,
                            CheckIfXCanGoRight<G, R, C + 1>>>::value value;
};

template <typename G>
struct CheckWin
{
    typedef typename FindCar<G, G::length - 1, G::width - 1, X>::value x_location;

    typedef typename CheckIfXCanGoRight<G, x_location::row, x_location::col>::value value;
