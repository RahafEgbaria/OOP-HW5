...
struct GetBack
{
        ...
        constexpr static int row =
            row_before_fix < 0 || row_before_fix >= G::length ? R : row_before_fix;
        constexpr static int col =
            col_before_fix < 0 || col_before_fix >= G::width ? C : col_before_fix;
...
struct MoveVehicle
{
        ...
        constexpr static bool legal_direction =
            (D == UP || D == DOWN) ? (board_cell::direction == UP || board_cell::direction == DOWN) : (board_cell::direction == LEFT || board_cell::direction == RIGHT);
